...
class DeviceRepository(BaseRepository):
    ...
    async def update_device_validate(self, device_id: UUID, user_id: UUID, **kwargs)-> bool:

        permitted_device = select(
            self.model.id
        ).where(
    ...
    async def soft_delete_one_by_id(self, entity_id: UUID, user_id: UUID) -> bool:
        ...
        query = (
            select(self.model).where(self.model.user_id == user_id, self.model.id == entity_id)
        )
